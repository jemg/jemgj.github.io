...
exports.datatoJSON = function(req, res) {
  ...
  exports.datatoJSON2 = function(req, res) {
    let sql = `SELECT Name, myTime.Employee_id, Approval_type, Duration, start_date, end_date, Approver_name, Approver_id FROM test.myTime JOIN test.Employee ON Employee.Employee_id = myTime.employee_id`; //query here 
    db.all(sql, [], (err, rows) => {
        if (err) {
          throw err;
        }
